...
const IPSTACK_KEY = '4962b38b0fe5e68add68b2e330e218e3';
...
const main = async () => {
  ...
  // DATA GETTERS
  const geoRes = await axios.get('http://api.ipstack.com/check?access_key=' + IPSTACK_KEY);
  const geo = { lat: geoRes.data.latitude, lon: geoRes.data.longitude };
  ...
  // MAP
  const map = grid.set(1, 0, 4, 4, contrib.map, { label: ' Current Location ' });
  ...
  // NET
  const netGraph = grid.set(5, 9.5, 7, 2.5, contrib.tree, { label: ' Interfaces ' });
  netGraph.focus();
  let itfs = {};
  net.map(
    itf =>
  ...
  const getIntervalData = async () => {
    ...
    try {
      ...
      cpuTemp.setData({ titles: cores.map((c, i) => `c${i + 1}`), data: cores });
      screen.render();
    } catch (err) {
      console.error(err);
    }

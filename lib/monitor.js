...
module.exports = async () => {
  ...
  const geoRes = await axios.get(
    'http://api.ipstack.com/check?access_key=' + IPSTACK_KEY,
  );
  ...
  grid.set(5, 3, 1, 2.5, blessed.box, {
    label: ' Memory ',
    content:
      (memInfo[0].type || 'No info') +
      ` • ${(initMem.total / 1000000000).toFixed(1)}${gbPrefix}`,
    align: 'center',
  });
  ...
  md.setMarkdown(
    '`System Monitor` ' +
      version +
      ' • Made by __BMK__ • Licensed under MIT license.',
  );
